...
After merging all the datasets above, the result data frame `PolEco_df` contains `r nrow(PolEco_df)` obervations of `r ncol(PolEco_df)` variables. The variable `year` ranges from `r min(pull(PolEco_df, year))` to `r max(pull(PolEco_df, year))`. And the names of key variables are `year` and `month`.